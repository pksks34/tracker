...
include("config.php");
$gr=$_POST["client"];
$sql = "SELECT * from billing where clientname='$gr'";
$result = mysqli_query($conn, $sql);

if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
        echo "<tr class='info'><td>".$row["date"]."</td><td>".$row["bill"]."</td><td>".$row["client"]."</td><td>".$row["nassignment"]."</td><td>".$row["detail"]."</td><td>".$row["amount"]."</td><td>".$row["service"]."</td></tr>";
    }
} else {
    echo "0 results";
}
